...
class ChartDefault {
  ...
  update(el, state) {
    let firstData = state.data, candidateNames = state.candidates;
  ...
  _drawPoints(el, data, candidates) {
    ...
    var x = d3.time.scale().range([0, width - padding]);
    ...
    x.domain(
      d3.extent(mergedData, function(d) {
        return d.date;
      })
    );
    y.domain(
    ...
    for (let i in data) {
      ...
      candidatePaths
        ...
        .attr(
          'transform',
          'translate(' +
            x(data[i][data[i].length - 1].date) +
            ',' +
            y(data[i][data[i].length - 1].amount) +
            ')'
        );
    }
  }
