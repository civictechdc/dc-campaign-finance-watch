...
export default class CandidateCard extends React.Component {
  ...
  componentWillUnmount() {
    window.removeEventListener(
      'resize',
      this.changeChart(this.state.activeChart)
    );
  }
      ...
      if (this.state.activeChart) {
        switch (this.state.activeChart) {
          case 'contributionOverTime':
            shapedData = ProcessContributionsOverTime(
              this.state.chartData.data,
              candidateName
            );
            break;
          case 'contributorBreakdown':
            shapedData = ProcessContributorBreakdown(
              this.state.chartData.data,
              candidateName
            );
            break;
          case 'contributorDendogram':
            shapedData = ProcessContributionsToTree(
              this.state.chartData.data,
              candidateName
            );
            break;
          case 'contributionByWard':
            shapedData = ProcessContributionByWard(
              this.state.chartData.data,
              candidateName
            );
            break;
    ...
    return (
      <div className="candidate-card">
        <Panel>
          ...
          <hr />
          <LinkContainer
            to={`candidate/${candidateId}/campaign/${data.campaignId}`}
          >
