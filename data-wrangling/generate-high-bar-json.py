...
output_dir = '../json'
ocf_retrieval_time = c_time = time.ctime(os.path.getctime(os.path.join(input_dir, input_file_name)))
...
def generate_grcont_json(df, year, office, ocf_retrieval_time):
...
def generate_cocont_json(df, year, office, ocf_retrieval_time):
  title = str(office) + ' (' + str(year) + ')'
...
for rownum in range(0, len(yo.index)):
  ...
  gcont = generate_grcont_json(dyo_data, year, office, ocf_retrieval_time)
  ccont = generate_cocont_json(dyo_data, year, office, ocf_retrieval_time)
  graphsjson = ''
