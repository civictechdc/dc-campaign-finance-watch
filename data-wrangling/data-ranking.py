...
df['some data'] = (pd.notnull(df['Employer Name']) |
                       df['Employer Name'].str.contains('none') |
                       df['Employer Name'].str.contains('unemployed') |
                       df['Employer Name'].str.contains('self') |
                       df['Employer Name'].str.contains('Self'))
...
subset = df[['Election Year', 'Office', 'Candidate Name', 'complete data', 'some data', 'donors']]
grouped = subset.groupby(['Election Year', 'Office', 'Candidate Name']).agg({'complete data' : np.mean, 'some data' : np.mean, 'donors' : np.sum }).reset_index()
...
sc['pad'] = np.where(sc['percent data complete']<9.5, ' ', '')
sc['percent data complete'] = sc['percent data complete'].round(0).map(int).map(str) + '%'
sc['percent data complete'] = sc['pad'] + sc['percent data complete']

sc['percent some data'] = sc['some data'] * 100
sc['percent some data'] = sc['percent some data'].round(0).map(int).map(str) + '%'
